...
# Select model to use
load_model_dir = './saved_models/resnet34/000/'  # Model directory to ProtoPNet. 
load_model_name = '15_15push0.7819.pth'# Model name of trained ProtoPNet


test_dir = './data/CUB_200_2011/dataset/' # Path to dataset
dataset = 'train_sub10'
test_dataset = 'test_sub10'
...
for path, subdirs, files in os.walk(os.path.join(test_dir, dataset)):
    for subdir in subdirs:
        ...
        # if count > max_count:
        #     break
        print("class: ", subdir, count, "/ 10",flush=True)
